...
async function main() {
  ...
  await t("logos_read: system tasks", async () => {
    const r = await readTool.execute({ uri: "logos://system/tasks" });
    if (!r.includes("tasks")) throw new Error(r.slice(0, 80));
  });

  await t("logos_write: sandbox file (relative)", async () => {
    await writeTool.execute({ uri: "test.txt", content: "from kairos enclave" });
  });

  await t("logos_read: sandbox file back", async () => {
    const r = await readTool.execute({ uri: "test.txt" });
  ...
  await t("logos_exec: echo", async () => {
    const r = await execTool.execute({ command: "echo hello-kairos" });
    if (!r.includes("hello-kairos")) throw new Error(r);
  });

  await t("logos_exec: cat sandbox file", async () => {
    const r = await execTool.execute({ command: "cat test.txt" });
  ...
  await t("logos_patch: users profile", async () => {
    ...
    const r = await readTool.execute({ uri: "logos://users/alice/profile.json" });
